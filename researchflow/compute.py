...
def _pid_alive(pid: int) -> bool:
    if pid <= 0:
        return False
    if os.name == "nt":
        # On Windows, os.kill(pid, 0) is not a POSIX-style existence probe and
        # can terminate the target process with exit code 0. Query the process
        # handle instead, without sending a signal.
        import ctypes
